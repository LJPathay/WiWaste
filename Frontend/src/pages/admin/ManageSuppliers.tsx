...
export function ManageSuppliers() {
  ...
  return (
    <div className="space-y-6 w-full font-sans">
      ...
      <DataTable
        columns={columns}
        data={paginatedSuppliers as (ApiSupplier & Record<string, unknown>)[]}
        rowKey={(row) => (row as ApiSupplier).id}
        selectable
        ...
        actions={(row) => (
          <>
            <ActionButton
              ...
              label="Edit"
              onClick={() => openEdit(row as ApiSupplier)}
            />
            <ActionButton
              icon={<Archive className="h-3.5 w-3.5" />}
              label="Delete"
              variant="danger"
              onClick={() => setArchivingSupplier(row as ApiSupplier)}
            />
          </>
        )}
        pagination={
