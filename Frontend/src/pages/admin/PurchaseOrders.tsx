    ...
    },
{
          ...
          minWidth: '100px',
          render: (row) => (
            <span className={`inline-block rounded-full px-2 py-0.5 text-[9px] font-medium ${statusColor[row.status] ?? ''}`}>{row.status}</span>
          ),
        },
    {
      key: 'created_at',
    ...
    <div className="space-y-4 w-full">
      <div className="flex items-center justify-between gap-3">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 bg-[#006a61] hover:bg-[#00574f] text-white px-3 h-8 rounded-lg text-xs font-semibold transition-colors"
        >
