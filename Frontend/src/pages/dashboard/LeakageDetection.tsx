  ...
  if (error) {
    return (
      <div className="space-y-3">
        <div className="rounded-xl border border-rose-200 bg-rose-50 p-4 text-rose-800 dark:border-rose-500/20 dark:bg-rose-500/10 dark:text-rose-200">
          <div className="flex items-center gap-1.5 font-bold">
            <AlertTriangle className="h-4 w-4" /> Could not load loss-risk data
          </div>
          <p className="mt-1 text-xs">{error}</p>
          <p className="mt-2 text-[9px] text-rose-600/70 dark:text-rose-300/70">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <button
            onClick={handleRun}
            disabled={running}
            className="inline-flex items-center gap-1.5 h-8 rounded-xl bg-rose-600 px-3 text-xs font-semibold text-white hover:bg-rose-700 disabled:opacity-50"
          >
      ...
      {empty ? (
        <div className="rounded-xl border border-dashed border-slate-300 bg-white p-6 text-center dark:border-white/10 dark:bg-slate-900">
          ...
          <button
            onClick={handleRun}
            disabled={running}
            className="mt-3 inline-flex items-center gap-1.5 h-8 rounded-xl bg-rose-600 px-3 text-xs font-semibold text-white hover:bg-rose-700 disabled:opacity-50"
          >
            <RefreshCw className={`h-3.5 w-3.5 ${running ? 'animate-spin' : ''}`} />
            {running ? 'Scoring…' : 'Run Risk Assessment'}
          </button>
...
<>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              <div className="rounded-xl border border-rose-100 bg-rose-50 p-3 dark:border-rose-500/20 dark:bg-rose-500/5">
                <div className="text-[9px] font-semibold uppercase tracking-widest text-rose-400 dark:text-rose-500">Total Expected Loss</div>
                <div className="mt-1.5 text-xl font-bold text-rose-700 dark:text-rose-300">{currencyFormatter.format(totalExpectedLoss)}</div>
                <div className="mt-0.5 text-[9px] text-rose-600/70 dark:text-rose-400/70">across {summary?.total_products ?? items.length} active SKUs</div>
              </div>
              <div className="rounded-xl border border-amber-100 bg-amber-50 p-3 dark:border-amber-500/20 dark:bg-amber-500/5">
                <div className="text-[9px] font-semibold uppercase tracking-widest text-amber-400 dark:text-amber-500">Highest Risk SKU</div>
                <div className="mt-1.5 text-lg font-bold text-amber-700 dark:text-amber-300">{topItem?.product_name ?? '—'}</div>
                <div className="mt-0.5 text-[9px] text-amber-600/70 dark:text-amber-400/70">
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-slate-900">
            <div className="flex items-center justify-between gap-3 flex-wrap mb-3">
              ...
              <span className="inline-flex items-center gap-1.5 text-[9px] rounded-full bg-rose-50 text-rose-700 px-2 py-0.5 font-semibold dark:bg-rose-500/10 dark:text-rose-400">
          ...
          {topItem && topItem.risk_tier === 'High' && (
            <section className="rounded-xl border border-rose-200 bg-gradient-to-r from-rose-50 to-orange-50 p-4 dark:border-rose-500/20 dark:from-rose-500/10 dark:to-orange-500/5">
              <div className="flex items-start gap-2.5">
                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-rose-100 dark:bg-rose-500/20">
                  <AlertCircle className="h-4 w-4 text-rose-600 dark:text-rose-400" />
                </div>
                <div>
                  <div className="flex items-center gap-1.5">
                    <h3 className="text-sm font-bold text-rose-900 dark:text-rose-200">Biggest Vulnerability: {topItem.product_name}</h3>
                    <span className="rounded-full bg-rose-100 px-1.5 py-0.5 text-[9px] font-bold text-rose-700 dark:bg-rose-500/20 dark:text-rose-300">PRIORITY</span>
                  </div>
                  <p className="mt-0.5 text-xs leading-5 text-rose-800/80 dark:text-rose-200/70">
