...
const initialItems: OverstockItem[] = [
  { id: 1, name: 'Del Monte Tomato Sauce 250g', category: 'Canned Goods', qtyOnHand: 1200, reorderPoint: 350, excessQty: 850, unitCost: 28, recommendedAction: 'Markdown & Sell', applied: false },
  { id: 2, name: 'Coca-Cola 1.5L', category: 'Beverages', qtyOnHand: 980, reorderPoint: 200, excessQty: 780, unitCost: 72, recommendedAction: 'Return to Supplier', applied: false },
  { id: 3, name: 'Safeguard Bar Soap 135g', category: 'Personal Care', qtyOnHand: 3400, reorderPoint: 700, excessQty: 2700, unitCost: 55, recommendedAction: 'Markdown & Sell', applied: false },
  { id: 4, name: 'Gardenia Classic White Bread', category: 'Bakery', qtyOnHand: 420, reorderPoint: 120, excessQty: 300, unitCost: 65, recommendedAction: 'Liquidate', applied: false },
  { id: 5, name: 'Nestlé Bear Brand Powdered Milk', category: 'Dairy', qtyOnHand: 1800, reorderPoint: 500, excessQty: 1300, unitCost: 145, recommendedAction: 'Return to Supplier', applied: false },
];
...
export function OverstockRisks() {
  ...
  function openConfirm(item: OverstockItem) { setConfirm({ open: true, item }); }

  function handleApply() {
    if (!confirm.item) return;
    setItems((prev) => prev.map((i) => (i.id === confirm.item!.id ? { ...i, applied: true } : i)));
